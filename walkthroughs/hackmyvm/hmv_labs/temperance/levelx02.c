...
char* toUpperCase(char* str) {
    char* ptr = str; // Keep a pointer to the start of the string
    while (*ptr != '\0') { // Loop until the end of the string
        *ptr = toupper((unsigned char)*ptr); // Convert the character to uppercase
...
int main() {
    ...
    const char *level = "02"; // Replace with the appropriate level number
    ...
    // Step 4: Convert the challenge string to uppercase using the function
    toUpperCase(challenge);
    printf("Converting string to uppercase: %s\n", challenge);

    // Step 5: Send the challenge back to the server
